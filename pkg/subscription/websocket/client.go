...
type Client struct {
	...
	readClientData     func(rw io.ReadWriter) ([]byte, ws.OpCode, error)
	writeServerMessage func(w io.Writer, op ws.OpCode, p []byte) error
}
...
func NewClient(
	...
	var (
		reader = wsutil.ReadClientData
		writer = wsutil.WriteServerMessage
	)

	if compression {
		reader = func(rw io.ReadWriter) ([]byte, ws.OpCode, error) {
			...
			if len(frame.Payload) == 0 || !frame.Header.OpCode.IsData() {
				return nil, ws.OpContinuation, nil
			}

			compressed, err := wsflate.IsCompressed(frame.Header)
			if err != nil {
				return nil, 0, fmt.Errorf("wsflate.IsCompressed: %w", err)
			}

			if compressed {
				frame, err = decompressMessage(frame)
				if err != nil {
					return nil, 0, fmt.Errorf("decompressMessage: %w", err)
				}
			}

			return frame.Payload, frame.Header.OpCode, nil
		}

		writer = func(w io.Writer, op ws.OpCode, p []byte) error {
			messageFrame := ws.NewTextFrame(p)
			frame, err := compressMessage(messageFrame)
			if err != nil {
				return fmt.Errorf("compressMessage: %w", err)
			}
...
func (c *Client) ReadBytesFromClient() ([]byte, error) {
	...
	data, opCode, err := c.readClientData(c.clientConn)
...
func (c *Client) WriteBytesToClient(message []byte) error {
	...
	err := c.writeServerMessage(c.clientConn, ws.OpText, message)
