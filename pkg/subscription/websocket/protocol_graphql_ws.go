...
func (p *ProtocolGraphQLWSHandler) Handle(
	ctx context.Context,
	...
	case GraphQLWSMessageTypeConnectionInit:
		...
		if err != nil {
			...
			return ctx, engine.TerminateAllSubscriptions(&p.writeEventHandler)
		}

		go p.handleKeepAlive(ctx)
	case GraphQLWSMessageTypeStart:
		return ctx, engine.StartOperation(ctx, message.Id, message.Payload, &p.writeEventHandler)
	case GraphQLWSMessageTypeStop:
		return ctx, engine.StopSubscription(message.Id, &p.writeEventHandler)
