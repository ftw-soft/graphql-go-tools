...
func (p *ProtocolGraphQLTransportWSHandler) Handle(
	ctx context.Context,
	...
	case GraphQLTransportWSMessageTypePong:
		return ctx, nil // no need to act on pong currently (this may change in future for heartbeat checks)
	case GraphQLTransportWSMessageTypeSubscribe:
		return ctx, p.handleSubscribe(ctx, engine, message)
	case GraphQLTransportWSMessageTypeComplete:
		return ctx, p.handleComplete(engine, message.Id)
	default:
		p.closeConnectionWithReason(
